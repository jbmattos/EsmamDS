'''
SCRIPT FOR PROCESSING THE RESULTS FROM < lr-rules.jar > EXECUTION INTO LOG-FILES 
...
def __get_jaccard_matrix(db_results, db, baseline, small=True):
    ...
    antecedents = db_results.apply(lambda x: x.split(' THEN ')[0]).apply(lambda x: x.split(': IF ')[1]).apply(lambda x: x.replace("{","")).apply(lambda x: x.replace("}","")).apply(lambda x: x.split(' AND ')).apply(lambda x: __get_antecedent(x))
...
def __get_rules_pval(db_results, db): 
    ...
    for rule in range(len(rules_idx)):
        ...
        group_id = ['rule'] * times_sg.shape[0] + ['cpm'] * times_comp.shape[0]
...
def __get_survival_models(folder, save_path):
    ...
    Generated files:
    >> file: lrrules_survival-models.json
    ...
    save_name = save_path+'lrrules_survival-models.json'
...
def __get_logrank_results(folder, save_path):
    ...
    The saved log-results refer to LR-Rules runs over all 14dbs.
    >> Generated files:
    1. file: lrrules_rules-pval.json
    ...
    2. file: lrrules_pval-matrix.json
    ...
    # saving processed files
    save_name = save_path+'lrrules_rules-pval.json'
    with open(save_name, 'w') as f:
        json.dump(rules_pval, f)
    print('..saved: {}'.format(save_name))

    save_name = save_path+'lrrules_pval-matrix.json'
    with open(save_name, 'w') as f:
...
def __get_jaccard_results(folder, save_path):
    ...
    The saved log-results refer to LR-Rules runs over all 14dbs.
    >> Generated files:
    1. file: lrrules_jaccard-cover.json
    ...
    2. file: lrrules_jaccard-descript.json
    ...
    # saving processed files
    save_name = save_path+'lrrules_jaccard-cover.json'
    with open(save_name, 'w') as f:
        json.dump(jac_cover, f)
    print('..saved: {}'.format(save_name))

    save_name = save_path+'lrrules_jaccard-descript.json'
    with open(save_name, 'w') as f:
...
def __get_metrics(folder, save_path):
    ...
    save_name = save_path+'lrrules_metrics.csv'
...
def run(folder, save_path):
    '''
    Script to extract processed results log-files for LR-Rules algorithm

    Parameters
    ----------
    folder : string
        Folder containing the < {dataset-name}_ruleSet.txt > output files from the algorithm.
    save_path : string
