'''
SCRIPT FOR PROCESSING THE RESULTS FROM < PySubgroup.py > EXECUTION INTO LOG-FILES 
...
def __get_jaccard_matrix(db_results, db, baseline, small=True):
    ...
    antecedents = db_results['subgroup'].apply(lambda x: x.replace("'","")).apply(lambda x: x.split(' AND ')).apply(lambda x: __get_antecedent(x))
...
def __get_rules_pval(db_results, db, baseline):
    ...
    for rule in range(len(rules_idx)):
        ...
        if baseline=='complement':
            cases_cpm = set(cases.iloc[rule])^set(db.index)
        ...
        group_id = ['rule'] * times_sg.shape[0] + ['comp'] * times_comp.shape[0]
...
def __get_models_matrix(db_results, db):
    ...
    queries = antecedents.apply(lambda x: ' and '.join([f'`{k}` == {repr(v)}' for k, v in x.items()]))
    cases = queries.apply(lambda x: set(db.query(x).index))
    rules = list(range(antecedents.shape[0]))
...
def __get_survival_models(folder, save_path):
    ...
    The saved log-results refer to LR-Rules runs over all 14dbs.
    Generated files:
    >> file: pysg_survival-models.json
        A dictionary containing the tables for the survival models [population and rules] of the LR-Rules results over each dataset. 
    ...
    save_name = save_path+'pysg_survival-models.json'
    with open(save_name, 'w') as f:
        json.dump(models, f)
    print('..saved: {}'.format(save_name))
    return

def __get_logrank_results(baseline, folder, save_path):
    '''
    Computes the [Rules X Rules] matrix of jaccard index for both coverage and description.
    The saved log-results refer to LR-Rules runs over all 14dbs.
    >> Generated files:
    1. file: pysg_rules-pval.json
    ...
    for db_name in DATASETS:
        ...
        rules_pval[db_name] = __get_rules_pval(db_results, db, baseline)
        pval_matrix[db_name] = __get_pval_matrix(db_results, db)
    
    # saving processed files
    save_name = save_path+'pysg_rules-pval.json'
    with open(save_name, 'w') as f:
        json.dump(rules_pval, f)
    print('..saved: {}'.format(save_name))

    save_name = save_path+'pysg_pval-matrix.json'
    with open(save_name, 'w') as f:
...
def __get_jaccard_results(folder, save_path):
    ...
    The saved log-results refer to LR-Rules runs over all 14dbs.
    >> Generated files:
    1. file: pysg_jaccard-cover.json
    ...
    2. file: pysg_jaccard-descript.json
    ...
    # saving processed files
    save_name = save_path+'pysg_jaccard-cover.json'
    with open(save_name, 'w') as f:
        json.dump(jac_cover, f)
    print('..saved: {}'.format(save_name))

    save_name = save_path+'pysg_jaccard-descript.json'
    with open(save_name, 'w') as f:
...
def __get_metrics(folder, save_path):
    '''
    Compute defined metrics for LR-Rules results over all 14 datasets and saves a .csv table (dataset X metrics)
    Generated file:
    >> pysg_metrics.csv
    '''
    ...
    for db_name in DATASETS:
        ...
        metrics[db_name]['length'] = antecedents.apply(lambda x: len(x)).mean()
        metrics[db_name]['rule_coverage'] = cases.apply(lambda x: len(x)).sum()/db_results.shape[0]/db.shape[0]
        metrics[db_name]['set_coverage'] = len(all_cases)/db.shape[0]
        metrics[db_name]['cr'] = __calc_CR(Counter(rules_cov), db.shape[0])
    
    save_name = save_path+'pysg_metrics.csv'
...
def run(algorithm, folder, save_path):
    '''
    Script to extract processed results log-files for Beam Search (BS) class algorithms

    Parameters
    ----------
    algorithm : string
        Name of the BS-class algorithm to process the results.
        [BS-EMM-pop, BS-EMM-cpm, BS-SD-pop, BS-SD-cpm]
    folder : string
        Folder containing the < {dataset-name}_result.csv > output files from the algorithm.
    save_path : string
    ...
    __get_logrank_results(baseline, folder, save_path)
